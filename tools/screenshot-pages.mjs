...
// height once fonts are ready, and the capture is cropped to that height.
...
// Usage: node tools/screenshot-pages.mjs [page-filter] [--slice]   (--slice also writes 1600px-tall pieces, easier to read)
...
const args = process.argv.slice(2); const slice = args.includes('--slice'); const filter = args.find(a => !a.startsWith('--'));
const widths = { phone: 390, tablet: 820, desktop: 1440 }; const H = 16000; const BG = [136, 136, 136];
...
for (const p of pages) {
  ...
  for (const [label, w] of Object.entries(widths)) {
    ...
    fs.writeFileSync(wrap, `<!doctype html><html><body style="margin:0;background:rgb(${BG.join(',')})"><iframe id="f" src="file://${src}" style="border:0;display:block;width:${w}px;height:${H}px"></iframe>\n<script>const f=document.getElementById('f');const fit=()=>{const d=f.contentDocument;if(d&&d.body)f.style.height=Math.ceil(d.body.getBoundingClientRect().bottom)+'px'};f.addEventListener('load',()=>{const d=f.contentDocument;(d&&d.fonts?d.fonts.ready:Promise.resolve()).then(()=>{fit();setTimeout(fit,100)})});</script></body></html>`);
    const raw = path.join(WRAP, `${name}-${label}.raw.png`); const out = path.join(OUT, `${name}-${label}.png`);
    execFileSync(CHROME, ['--headless=new', '--disable-gpu', '--hide-scrollbars', '--no-first-run', '--allow-file-access-from-files', `--window-size=${Math.max(w, 600)},${H}`, '--virtual-time-budget=6000', `--screenshot=${raw}`, 'file://' + wrap], { stdio: 'ignore' });
